...
module.exports = function( grunt ) {
    ...
    // our terminal for jshint, validatealljs_core task will run with:
    // $ grunt validatealljs_core

    var taskDescriptions = {
        validatealljs_core: "Validate all JS files for the rules defined in `.jshintrc` file.",
        validatejs_core: "Prompt to take JS file names in a Textbox and validate those files with rules defined in `.jshintrc` file.",
        validateselectedjs_core: "Prompt to take JS file names with Checkboxes and then validate selected files with the rules defined in `.jshintrc` file.",
        validatejslines_core: "Prompt to take JS file name with Line Numbers to validate the provided lines with rules defined in `.jshintrc` file.",
    ...
    // Task for validating single JS file using "grunt validatejs_core" command.
    grunt.registerTask( "validatejs_core", taskDescriptions.validatejs_core, function() {
        grunt.task.run(["prompt:ask_js_name", "task_prompt_js"]);       
    });

    // Task for validating single JS file using "grunt validateselectedjs_core" command.
    grunt.registerTask( "validateselectedjs_core", taskDescriptions.validateselectedjs_core, function() {
    ...
    // Task for validating all the JS files using "grunt validatealljs_core" command.
    grunt.registerTask( "validatealljs_core", taskDescriptions.validatealljs_core, function() {
        console.log(grunt.config("jsFilesToBeValidated"));
    ...
    grunt.registerTask( "validatejslines_core", taskDescriptions.validatejslines_core, function() {
    ...
    grunt.registerTask( "task_prompt_js_lines", taskDescriptions.task_prompt_js_lines, function() {
        ...
        if(userInput.length > 0) {
            ...
            // Run the grunt "jshint" task manually from code with above set configurations.
            if(grunt.config("removeTempFile") === true) {
